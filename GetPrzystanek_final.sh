...
#$1 przystanek (wielkimi literami) | stop (upper-case)
...
STEPONE=$(wget -O- -q "http://wyszukiwarka.ztm.waw.pl/bin/stboard.exe/pnx?input=$1&boardType=dep&time=$2%2B1&productsFilter=1111110111\&date=$3&maxJourneys=$4&start=yes" | head -n-15 )
#echo "$STEPONE" > STEPONE.txt
STEPONEHALF=$(echo "$STEPONE" | wget --ignore-tags=link,img -i- -q --force-html -r -O- -np | sed -e ':a;N;$!ba;s/ <\/td>\n/ /g' -e 's/<[^>]*>//g' -e 's/&lt;/</g' -e 's/&gt;/>/g' -e 's/&amp;/\&/g' -e 's/&#322;/l/g' -e 's/ \{1,\}/ /g' -e 's/&#379;/Z/g' -e 's/&#321;/L/g' -e 's/&#323;/N/g' -e 's/&#280;/E/g' -e 's/&#211;/O/g' -e 's/&#260;/A/g' -e 's/&#346;/S/g' | egrep -a -e "N*[0-9]{1,} do [A-Z0-9 +.-]*" -e "$1")
